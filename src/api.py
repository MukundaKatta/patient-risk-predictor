"""FastAPI application for patient-risk-predictor."""
...
app = FastAPI(title="patient-risk-predictor", version="0.1.0", description="patient-risk-predictor API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "patient-risk-predictor", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "patient-risk-predictor", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "patient-risk-predictor", "version": "0.1.0", "ready": True,
